...
async def start(client, message):
                ...
                await client.send_cached_media(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            ...
                            [
                                InlineKeyboardButton('📌 ᴊᴏɪɴ ᴜᴘᴅᴀᴛᴇꜱ ᴄʜᴀɴɴᴇʟ 📌', url=f'https://t.me/Best_Movie_Channel7') #Don't change anything without contacting me @LazyDeveloperr
                            ]
                        ]
                    )
                )
    ...
    if data.startswith("sendfiles"):
        ...
        k = await client.send_message(
            chat_id=user_id,
            text=f"<b>📕Nᴀᴍᴇ ➠ : <code>{files.file_name}</code> \n\n🔗Sɪᴢᴇ ➠ : {get_size(files.file_size)}\n\n📂Fɪʟᴇ ʟɪɴᴋ ➠ : {g}\n\n<i>Note: This message is deleted in 10 mins to avoid copyrights. Save the link to Somewhere else</i></b>\n\n",
            reply_markup=InlineKeyboardMarkup(
                [[
                    InlineKeyboardButton('📂 Dᴏᴡɴʟᴏᴀᴅ Nᴏᴡ 📂', url=g)
                ], [
        ...
        for file in files:
            ...
            if not await db.has_premium_access(message.from_user.id) and not await check_verification(client, message.from_user.id) and VERIFY == True:
                btn = [[
